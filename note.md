...
- \n:换行
    - 换行
- \t:制表符
  - 水平制表符。用于在输出中插入一个水平制表位，通常相当于4或8个空格，具体取决于环境设置
- \b:退格
  - 将光标向后移动一个位置
- \r:回车
  - 将光标移至行首
...
#### 定义:
```cpp
short j;     // 短整型 -32768 ~ 32767
int n;       // 整形   -2147483648 ~ 2147483647
long long m; // 长整形 -9223372036854775808 ~ 9223372036854775807
```
### 浮点数:
- 用于存储小数的类型
#### 定义:
...
### 类型转换
#### 1.隐式类型转换(在表达式中自动发生的,无需进行任何明确的指令或函数调用)
```c++
int i = 10;
float f = 3.14;
double d = i + f; // 隐式将int类型转换为double类型
```
#### 2.显式类型转换(需要使用强制类型转换运算符)
```c++
